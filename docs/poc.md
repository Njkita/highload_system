...
| redpanda | 0.5 | 768 M |
| redis | 0.15 | 256 M |
| catalog | 0.30 (iter-0..2) / 0.20 (iter-3 ×2) | 256 M / 192 M |
| order | 0.30 (iter-0..2) / 0.20 (iter-3 ×2) | 256 M / 192 M |
| payment | 0.20 | 256 M |
| psp-mock | 0.10 | 64 M |
| nginx gateway | 0.15 → 0.5 (с iter-1) | 96 M → 256 M (с iter-1) |

Суммарно на iter-0: ~2.2 vCPU / 3.4 GB. На iter-1..2 после поднятия gateway: ~2.55 vCPU / 3.5 GB. На iter-3 (×2 catalog + ×2 order, лимиты на каждый инстанс половинятся): ~2.55 vCPU / 3.7 GB. Остальное — ОС и page-cache БД.

Gateway получил больше CPU/RAM начиная с iter-1, потому что iter-0 показал: на 600+ RPS read под 96 MiB nginx упирается в proxy_buffers + access_log буфера на keepalive-соединениях, а 0.15 cpu становится потолком. Подробности — в [optimization-log.md](optimization-log.md#iteration-0--baseline).
...
iter-0   baseline: без индексов, без Redis-кэша, pgxpool=4, default Postgres,
         gateway 96M (worker_processes 2 — иначе на 64-ядерном хосте OOM)
iter-1   + 003_indexes.sql, тюнинг Postgres, pgxpool=16/20/10,
         + gateway 256M / 0.5 vCPU + worker_processes auto
iter-2   + Redis cache-aside на каталоге (CACHE_ENABLED=true)
iter-3   + горизонтальное масштабирование catalog/order ×2 за nginx,
         + nginx least_conn для catalog_up и order_up
```
