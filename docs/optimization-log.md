...
| Метрика | NFR (ДЗ 1) | iter-0 | iter-1 | iter-2 |
|---|---|---:|---:|---:|
| Read p99 (search + card) | < 500 ms | **470 ms** | 180 ms | 65 ms |
| Read max RPS | ≥ 100 | 180 | 540 | 980 |
| Write p99 (create + pay) | < 1000 ms | **720 ms** | 380 ms | 360 ms |
| Write max RPS | ≥ 30 | 45 | 110 | 120 |
| Error rate (5 min @ target) | < 1% | 0.4% | 0.2% | 0.1% |
| 2x spike error rate | < 5% | 12% | 3.8% | 1.6% |
| CPU (на пике) | 70–90% | 100% (postgres) | 78% | 85% |
| RAM (на пике) | — | 2.1 GB | 2.7 GB | 3.4 GB |
| Bottleneck | — | pgxpool + Seq Scan | HDD WAL fsync | Go GC + nginx upstream |
| NFR ДЗ 1 достигнут? | — | нет (search) | да | да |
| Минимальная планка? | — | да (read) / нет (spike) | да | да |
