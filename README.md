...
├── diagrams/                  # C4 L1/L2 + 3 sequence + deployment (.mmd + .svg + .png)
...
└── operations.md              # ДЗ 4 — deployment, observability, availability, TCO
...
- Развёртывание, список сервисов и роли — [operations.md §1](docs/operations.md#1-развёртывание); deployment diagram в [docs/diagrams/deployment.mmd](docs/diagrams/deployment.mmd).
- Стратегия деплоя (CI/CD, Canary/Rolling per service, probes, Expand/Contract миграции) — [operations.md §2](docs/operations.md#2-стратегия-деплоя).
- Observability (4 алерта Golden Signals, 3-уровневые дашборды, политика логов и PII) — [operations.md §3](docs/operations.md#3-observability).
- Доступность (SLO по доменам, RPO/RTO по компонентам, резервирование, maintenance window) — [operations.md §4](docs/operations.md#4-доступность).
- TCO в Yandex Cloud (текущая нагрузка + рост 2x/5x, операционные затраты) — [operations.md §5](docs/operations.md#5-tco-в-yandex-cloud).
